...
###############################
plot(density(as.vector(model.estimates[[2]])))
# ---------------------------------------------------------------- #
#---                  Model Predictions                         ----
# ---------------------------------------------------------------- #
model = apollo_loadModel("car_purchase_mxl")
...
#############################################################################
# Add alternative with attribute range7, price7, size7, fuel7, type7 ########
#############################################################################
ctypev <- c("van","regcar","truck","sportsv","stawagon","sportcar")
sizev <- c(0,1,2,3) 
fuelv <- c("cng", "electric","methanol", "gasoline")

data.list <- lapply(1, function(x) {
  nrep <- 1;
  range7 <- c(replicate(nrep,(runif(4654,100,300))));
  price7 <- c(replicate(nrep,(runif(4654,0.5,17.4))));
  data.frame(range7, price7);
});

database$range7 <-  as.numeric(as.character(trunc(data.frame(data.list[[1]])[,1])))
database$price7 <-  as.numeric(as.character(data.frame(data.list[[1]])[,2]))
database$size7 <- as.numeric(as.character(sample(sizev, size = nrow(database), replace = TRUE)))
database$fuel7 <- sample(fuelv,  size = nrow(database), replace = TRUE) 
database$type7 <- sample(ctypev,  size = nrow(database), replace = TRUE) 
...
write.csv(final,"Predictions_MXL_model1.csv", row.names = FALSE)
